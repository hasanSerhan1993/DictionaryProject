...
public class FXMLNewObjectController implements Initializable {
     ...
     */
    List<String> classesNames=Generator.getClassNames();
    List<TextField> attrField=new ArrayList<TextField>();
    List<Fields> objectAttributes=new ArrayList<Fields>();
...
//                 
                ...
                try {
                   ...
                   // Logger.getLogger(Projects.class.getName()).log(Level.SEVERE, null, ex);
                    ...
                    System.out.println("hasdhda");
                    for (i = 0; i < field.length; i++) {System.out.println(field[i].getType().getSimpleName());
                        System.out.println(classesNames);
                        attributesPlace.add(new Text(field[i].getName()), 1, i+2);
                        if(classesNames.contains(field[i].getType().getSimpleName()))
                            {//System.out.println("combo");
                                ComboBox cc=new ComboBox();
                                cc.getItems().addAll(Generator.getClassObject(field[i].getType().getSimpleName(), pool));
                                cc.getItems().add("null");
                            Fields F=new  Fields(cc);
                            objectAttributes.add(F);
